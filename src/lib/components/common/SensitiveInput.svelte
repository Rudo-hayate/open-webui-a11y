...
<div class={outerClassName}>
	...
	<input
		{id}
		class={`${inputClassName} ${show ? '' : 'password'} ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : ' outline-hidden placeholder:text-gray-700 dark:placeholder:text-gray-600'}`}
		{placeholder}
